...
## 5.3 👩‍💻 Semanur Buhan – Veri Yapılarının Optimizasyonu

### Görev Tanımı
Bu hafta kapsamında, C++ backend tarafında kullanılan veri yapılarının bellek kullanımı analiz edilmiş; gereksiz kopyalar önlenmiş ve daha verimli yapılara geçilmiştir. Değişikliklerin etkisi Valgrind ile ölçülerek kanıtlanmıştır.

---

### Yapılan Çalışmalar

**1. `enum class Durum` ile Tip Daraltma (`backend/VeriHavuzu.h`, `backend/VeriHavuzu.cpp`)**
- `AnalizVerisi::durum` alanı `std::string`'den `enum class Durum : uint8_t`'e dönüştürüldü.
- Sonuç: `sizeof(AnalizVerisi)` **48 bayt → 24 bayt** (-%50 küçülme). Her havuz nesnesinde 24 bayt tasarruf. 8 nesneli havuzda toplam **192 bayt** gereksiz bellek tamamen kaldırıldı.
- `durumStr()` yardımcı fonksiyonu `std::string_view` döndürür; heap tahsisi yoktur.

**2. `std::string_view` ile Gereksiz Kopya Önleme (`backend/VeriHavuzu.h`)**
- `AnalizVerisi::yazdir(const std::string& etiket)` imzası `yazdir(std::string_view etiket)` olarak değiştirildi.
- Sonuç: `yazdir("Iterasyon_0")` gibi string literal çağrılarında geçici `std::string` nesnesi **oluşturulmaz**, dolayısıyla heap tahsisi gerçekleşmez.

**3. `std::unique_ptr` ile Güvenli Sahiplik (`backend/VeriHavuzu.h`, `backend/VeriHavuzu.cpp`)**
- `std::vector<AnalizVerisi*>` → `std::vector<std::unique_ptr<AnalizVerisi>>` geçişi yapıldı.
- `nesneAl()` → `std::unique_ptr<AnalizVerisi>` döndürür; sahiplik çağırana devredilir.
- `nesneBirak()` → `unique_ptr` (move) alır; kapasite doluysa `delete` çağırmaya gerek kalmadan unique_ptr scope'tan çıkınca otomatik siler.
- `~VeriHavuzu()` içindeki manuel `for (AnalizVerisi* p : havuz_) delete p;` döngüsü kaldırıldı → `= default` yeterlidir.
- `new AnalizVerisi()` → `std::make_unique<AnalizVerisi>()` (exception-safe).

**4. `HavuzIstatistik` Struct ile Veri Yerelliği (`backend/VeriHavuzu.h`)**
- 3 ayrı `std::size_t` istatistik üyesi tek bir `HavuzIstatistik` struct'ı altında toplandı.
- Sonuç: İstatistik alanları bellekte bitişik durumda; CPU önbellek (cache) erişimi daha verimli.

**5. Kalıcı Test Dosyası (`tests/veri_havuzu_test.cpp`)**
- Optimize edilmiş API'yi doğrulayan, Valgrind ile çalıştırılabilir test dosyası eklendi.

---

### Ölçüm — Valgrind Karşılaştırması

| Metrik | Optimizasyon Öncesi | Optimizasyon Sonrası |
| :--- | :---: | :---: |
| `sizeof(AnalizVerisi)` | **48 bayt** | **24 bayt** |
| Toplam heap tahsisi | 77,904 bayt | 77,880 bayt |
| Tahsis sayısı | 4 | 4 |
| Serbest bırakma sayısı | 4 | 4 |
| Bellek sızıntısı | 0 | 0 (unique_ptr garantisi) |
| Hata sayısı | 0 | 0 |
...
Dört teknik birlikte uygulanmış; struct boyutu -%50 küçültülmüş, gereksiz kopyalar kaldırılmış ve bellek güvenliği derleme zamanına taşınmıştır.

**Güncellenen dosyalar:** `backend/VeriHavuzu.h`, `backend/VeriHavuzu.cpp`, 
...
## 6.3 👩‍💻 Semanur Buhan – Proje Dokümantasyonu Tamamlama

Görev Tanımı:
Akıllı Bellek Yönetimi projesinin geliştirme süreçleri tamamlandıktan sonra; sistemin mimarisini, çalışma mantığını, kod açıklamalarını ve kullanım adımlarını içeren kapsamlı teknik dokümantasyonun hazırlanması ve siteye entegre edilmesi.

Yapılan Çalışmalar:

Teknik İçerik Geliştirme: Sistem mimarisi, C++ backend ve Python frontend arasındaki haberleşme mantığı akademik ve teknik standartlara uygun olarak detaylandırıldı.

Kılavuz Hazırlığı: Kullanıcılar ve geliştiriciler için adım adım kurulum yönergeleri ile proje dizin yapısının (özellikle "tests" ve "analiz" klasörlerinin işlevlerinin) kullanımını içeren rehberler oluşturuldu.

**Siteye Entegrasyon: Hazırlanan bu detaylı proje dokümantasyonu, herkesin erişebilmesi ve projenin incelenebilmesi için resmi olarak siteye yüklendi**.

### Sonuç 
Proje dokümantasyonu **eksiksiz olarak tamamlanmış ve siteye başarılı bir şekilde eklenmiştir**. Sistem yüklemeye ve incelemeye hazırdır.
...
### Aylin Akagündüz
Projenin 6 haftalık çalışmalarını kapsayan 12 slaytlık final sunumu hazırlandı. Teknik sonuçlar, ekip katkıları ve gelecek planları sunuma dahil edildi.
